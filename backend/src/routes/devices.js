...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // FIX: Trim whitespace from version parameter to prevent URL encoding issues
    const cleanVersion = version ? version.trim() : version;
    const binaryUrl = `${protocol}://${host}/api/v1/devices/firmware/binary?device_uuid=${device_uuid}&version=${cleanVersion}`;
